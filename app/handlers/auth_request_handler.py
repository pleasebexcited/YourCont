...
It reads the browser form, asks AuthLogicHandler to create or check an account, then returns a template or redirect.
HTTP details stay here so they match the request handler layer on my class diagram and stay out of the password rules.
"""
...
class AuthRequestHandler(BaseRequestHandler):
    ...
    def signup(self):
        ...
        A validation or database problem will show the form again with a flash message and the values already typed, so people do not start from a blank page after a small mistake.
