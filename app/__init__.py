"""
app/__init__.py: Builds and sets up my Flask app
...
sets the log in cookie rules, turns on form protection,
then connects every page address from routes.py.
application.py calls create_app so the site starts the same way on my PC and later on Elastic
Beanstalk.
"""
...
def create_app():
    ...
    Paths, secrets, cookie flags, CSRF,
    and routes are all wired here so application.py only needs to call create_app().
    ...
    # Loading .env from the project folder (with override) means SECRET_KEY and DATABASE_URL win,
    # even when the terminal inherited an empty value from Windows.
    ...
    # Off for localhost, on for Elastic Beanstalk.
